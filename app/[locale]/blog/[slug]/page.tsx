 ...
 * Localised blog post page — /en/blog/[slug] and /th/blog/[slug]
 *
 * Phase B note: Blog body content is currently English-only.
 * Thai body (_th) will be wired in when the clinical translator delivers
 * translated blog posts. The localise() function in i18n-fallback.ts
 * already handles the graceful fallback when body_th is absent.
 */
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      title: post.title,
      description: post.metaDescription,
      url: `https://samuihomeclinic.com/${locale}/blog/${post.slug}`,
...
export default async function LocaleBlogPostPage({
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://samuihomeclinic.com/${locale}/blog/${post.slug}`,
    },
      ...
      <div className="min-h-screen bg-[#fbfbfb] font-sans">
        ...
        <nav className="sticky top-0 z-50 w-full bg-[#fbfbfb]/90 backdrop-blur-md border-b border-slate-200/60">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
            <Link
              href={`/${locale}/?catalogue=open#blog`}
              ...
              {headings.length > 0 && (
                <div className="lg:hidden mb-8 p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
                  ...
                  <ol className="space-y-2">
                    {headings.map((h, i) => (
                      <li key={i}>
                        <a href={`#${h.id}`} className="flex items-start gap-2.5 text-sm text-slate-600 hover:text-[#3eb5bd] transition-colors">
                ...
                {headings.length > 0 && (
                  <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
                    ...
                    <ol className="space-y-2.5">
                      {headings.map((h, i) => (
                        <li key={i}>
                          <a href={`#${h.id}`} className="flex items-start gap-3 text-sm text-slate-600 hover:text-[#3eb5bd] transition-colors leading-snug">
                            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#edf9fa] text-[#3eb5bd] text-xs font-bold flex items-center justify-center mt-0.5">
                              {i + 1}
                            </span>
                            {h.text}
                ...
                {related.length > 0 && (
                  <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
                    ...
                    <div className="space-y-4">
                      {related.map((r) => (
                        <Link key={r.slug} href={`/${locale}/blog/${r.slug}`} className="flex gap-3 group">
